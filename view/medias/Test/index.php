...
<?php echo HTML::JS("jMyCarousel.min"); ?>

<?php echo HTML::CSS("hoverScroll/smoothDivScroll"); ?>
<?php echo HTML::JS("scroller/jquery.mousewheel.min"); ?>
<?php echo HTML::JS("scroller/jquery.kinetic.min"); ?>
<?php echo HTML::JS("scroller/jquery.smoothdivscroll-1.2"); ?>
  ...
  .albumSlider { width: 120px; overflow: hidden; }
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('.albumThumbnails').live('mousemove', function(e) {
                  $(this).css('overflow-x', 'hidden');
                  var offset = $(this).offset();
                  var mouseXPos = e.pageX - offset.left;
                  var mouseYPos = e.pageY - offset.top;
                  var divHeight = $(this).height();
                  // console.log($(this).height());
                  // console.log('( e.pageX, e.pageY ) : ( ' + e.pageX + ', ' + e.pageY + ' )');
                  // console.log('( x, y ) : ( ' + (e.pageX - offset.left) + ', ' + (e.pageY - offset.top) + ' )');

                  var test = divHeight/2;
                  if(mouseYPos <= (divHeight/2-10)){
                    console.log('mouse on top');
                    var test2 = test / mouseYPos;
                    if(test2 > 2){

                    }
                    var leftPos = $(this).scrollTop();
                    console.log(test2);
                    (test2 > 2) ? test2 = 2 : test2 = test2;
                    $(this).scrollTop(leftPos - (10*test2));
                    // $(this).animate({
                    //     scrollTop: -100
                    // }, 800);
                  } else if(mouseYPos >= (divHeight/2+10)) {
                    console.log('mouse on bottom');
                    var test2 = mouseYPos / test;
                    var leftPos = $(this).scrollTop();
                    console.log(test2);
                    $(this).scrollTop(leftPos + (10*test2));
                    // $(this).animate({
            ...
            $('a.mdTrigger, figcaption').live('click', function(event){
                ...
                if(isAlbum){
                  $(this).find('a.gridTrigger').trigger('click');
                  return false;
                }
            ...
            $(".modal-content").on("resize", function(event, ui) {
                ...
                if($('div.albumThumbnails').length && $inAlbum){
                  $('div.albumThumbnails').find('ul:first').hide();
                  $('div.albumThumbnails').removeAttr('style');
                }
            });

            $(".modal-content").on( "resizestop", function() {
              $inAlbum = $(this).find('div.modal-body').find('div.imgLightbox').hasClass('albumLightbox');
              $('div.albumThumbnails').find('ul:first').show();
              if($('div.albumThumbnails').length && $inAlbum){
                console.log('RESIZE');
                // for (var i = 0; i <= $('.albumThumbnails a.mdTrigger').length; i++) {
                //   $('.albumThumbnails a.mdTrigger:nth-child('+i+')').find('img.photo').removeAttr('width').removeAttr('height').addClass('col-md-14'); 
                // };
                // var thumbnailsHeight  = $('.albumThumbnails').find('a.mdTrigger:first').find('img.photo').height(); 
                // thumbnailsHeight = thumbnailsHeight * $('div.albumThumbnails li').length;
                // $(this).find('div.modal-body').find('div.albumThumbnails').css('height', ($('.modal-body').height())+'px');
                // $('div.modal-body').find('.albumSlider').jMyCarousel({  
                //   visible: ''+thumbnailsHeight+'px', 
                //   vertical : true,
                //   eltByElt: true,
                //   mouseWheel : true,
                //   circular : false
            ...
            $('#imgModal').live('hide.bs.modal', function () {
                ...
                // console.log(curViewedImg.html());
                $('.row').animate({
                    scrollTop: curViewedImg.offset().top - $('.row').offset().top + $('.row').scrollTop()
                });
            });
            ...
            function PrevNextImg(elem){
              if($(elem).attr('id') == "prevImg"){
                ...
                prevInAlbum = curViewedImg.prev().find('a.mdTrigger').hasClass('album');
                console.log(prevInAlbum);
                console.log(curViewedImg.prev());
                if(!fullscreen){
                LoadModalContent(curViewedImg.prev().find('a.mdTrigger'), false, prevInAlbum);
                } else{
                ...
                nextInAlbum = curViewedImg.next().find('a.mdTrigger').hasClass('album');
                console.log(nextInAlbum);
                console.log(curViewedImg.next());
                if(!fullscreen){
                  LoadModalContent(curViewedImg.next().find('a.mdTrigger'), false, nextInAlbum);
                } else{
            ...
            function LoadModalContent(imgLinkTrigger, isAlbum, inAlbum){
                ...
                if(!isAlbum){
                  ...
                  $.get(formatedUrl, function(data){
                    ...
                    if(!exists(inAlbum) || !inAlbum){
                    ...
                    } else if(exists(inAlbum) && inAlbum){
                      var $albumThumbnails = $( 'div.og-expander-inner' ).find('div.albumThumbnails').html();
                      $('div.imgLightbox').after('<div class="albumThumbnails"></div>');
                      $('.modal-body').find('div.albumThumbnails').show();
                      $('.modal-body').find('div.albumThumbnails').addClass('col-md-1').css('z-index', '20');
                      $('.modal-body').find('div.albumThumbnails').html($albumThumbnails);
                      $('.modal-body').find('div.albumThumbnails').css({
                                                        'height': ($('.modal-body').height())+'px',
                                                        'width': '120px' ,
                                                        'overflow': 'auto'
                                                                      });
                      $('.modal-body').find('div.albumSlider').css({
                                                        'width': '120px' ,
                                                        'overflow': 'hidden'
                                                                      });
                      // -------------------------------------------------------------------------------// 
                      ...
                      $('div.modal-body').find('.albumThumbnails').wrapInner( '<div class = "albumSlider"></div>' ); 
                      // for (var i = 0; i <= $('.albumThumbnails a.mdTrigger').length; i++) {
                      //   $('.albumThumbnails a.mdTrigger:nth-child('+i+')').wrap( '<li></li>' ); 
                      // };
                      // for (var i = 0; i <= $('.albumThumbnails a.mdTrigger').length; i++) {
                        // $('div.modal-body').find('.albumThumbnails a.mdTrigger:nth-child('+i+')').find('img.photo').attr('width', '100px').removeAttr('height'); 
                        // $('.albumThumbnails a.mdTrigger:nth-child('+i+')').find('img.photo').attr('width', ''+($('div.albumThumbnails').width())+'px').removeAttr('height'); 
                        // $('.albumThumbnails a.mdTrigger:nth-child('+i+')').find('img.photo').attr('width', '100%').removeAttr('height'); 
                        // $('.albumThumbnails a.mdTrigger:nth-child('+i+')').find('img.photo').removeAttr('width').removeAttr('height'); 
                      // };

                      if(!$('.albumThumbnails').find('div.albumSlider').length){
                        // $('div.modal-body').find('.albumThumbnails').wrapInner( '<ul></ul>' ); 
                        // $('div.modal-body').find('.albumThumbnails').wrapInner( '<div class = "albumSlider"></div>' ); 
                        // $('div.modal-body').find('.albumSlider').jMyCarousel({  
                        //     visible: ''+($('.modal-body').height())+'px', 
                        //     vertical : true,
                        //     eltByElt: true,
                        //     mouseWheel : true,
                        //     circular : false
                        ...
                        // $('div.modal-body').find('.albumThumbnails').smoothDivScroll({
                        //   autoScrollingMode: "onStart"
                        // });
                      }
                      //---------------------------------------------------------------------------------------//
                      $('.modal-body').find('div.imgLightbox').addClass('col-md-8');
                      $('.modal-body').find('div.imgLightbox').addClass('albumLightbox');
                      $('.modal-body').find('div#comments').addClass('col-md-3').css('z-index', '30');
                    }
                  ...
                  // $(".modal-body").append('<div class="albumThumbs"></div>');
                  // $(".modal-body").append(albumThumbnails);
                  ...
                  //       $('.modal-body li:last').find('span').html($imageInfo.user);
                  //       $('.modal-body li:last').find('h3').html("");
                  //       $('.modal-body li:last').show();

                  //       $('.modal-body li:last').find('a.mdTrigger:first').clone().appendTo('.albumThumbnails');
