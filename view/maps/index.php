...
//*********************************************************************************************************************//
//-----------------------------------------end of svg icons and params-------------------------------------------------//
//*********************************************************************************************************************//


    //regions and areas and current and closest positions variables
    var curRegion, localRegion, curArea, curLocation, curInfoMarker = "";
    var cookieGeoloc = true, navigatorGeoloc = true, ipGeoloc = true, geoloc = true, checkingAdresse = false, startedIpGeoloc = false;
    ...
    function changeBounds($addBound, $customHomeAdress){
      ...
      for (var markers in markerArray) {  
        if(curRegion == markerArray[markers].markerRegion){
          ...
          foundBoundsMarkers = true;
        }
      }
      map.setCenter(bounds.getCenter());
      if(foundBoundsMarkers){
...
//*********************************************************************************************************************//
    ...
    function PromptCurrentPosition($adress){
      ...
      if(exists($.cookie('home')) && cookieGeoloc){
        ...
        for(coords in curLocation[0].geometry.location){
          if(curObjectCount == 0){
            curLocation['Lat'] = curLocation[0].geometry.location[coords];
          } else if(curObjectCount == 1){
            curLocation['Lng'] = curLocation[0].geometry.location[coords];
          }
          curObjectCount++;
        }
        curObjectCount = 0;
        curLocation['LatLng'] = new google.maps.LatLng(parseFloat(curLocation['Lat']), parseFloat(curLocation['Lng']));
      ...
      if(navigatorGeoloc || ipGeoloc){
        getCurrentPosition(function($location){
          ...
          findmarkers(findRegion($location), false, false, false);
        });
      } else {
        if(exists($adress)){
          $('#loader').clone().appendTo('div#localAdress').show();
          codeLatLng($adress, function($location){
            $('div#localAdress').find('#loader').hide();
    ...
    function getCurrentPosition($callback){ 
      if(navigator.geolocation && navigatorGeoloc) {
        ...
        setTimeout(function(){
          if(!startedIpGeoloc && !navigatorGeoloc){
            $('div#localAdress').show();
            $('p.manualAdress').show();
            showError($('#mapErrors'), 'Géolocalisation Impossible. Tentative de géolocalisation avec adresse');  
          }
        }, 9000);
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(function(position) {   
            ...
            if(error.code == 1){
              ...
              navigatorGeoloc = false;
              showError($('#mapErrors'), 'Géolocalisation Impossible. Tentative de géolocalisation avec adresse IP');
              PromptCurrentPosition();
...
//*********************************************************************************************************************//
    ...
    function setDirectionMarkers($marker,$markerAdress, $markerColor, $markerType){
      ...
      if(typeof $start.marker != "undefined" && typeof $end.marker != "undefined") { 
        ...
        $('div#directionTuto div#directionClick').hide();
        $('div#directionTuto div#directionAdd').show();
        $('div#directionTuto div#directionLaunch').show();
    ...
    function removeDirection() {       
      ...
      regionsClickedTemp = regionsClicked;
      regionsClicked = [];
      for(region in regionsClickedTemp){
        if(region != "insert" && region != "peek"){
          markerClustererArray[region].clearMarkers();
          findmarkers(regionsClickedTemp[region], true); 
        }
      }
    }
...
//*********************************************************************************************************************//
    ...
    function setClosestMarkers(){
      $('#loader').clone().appendTo('div.closetMarker').show();
      ...
      getClosestMarkers(function(){
        ...
        for(var nearMarker in nearestMarkers){
          ...
          if($markerInfo.event != 'event'){
            if(exists($markerInfo.title)){
              var markerTitle = "<h4><trong>"+$markerInfo.title +" </strong></h4><em>"+" - "+$markerInfo.club +"</em><br>";
            } else{
              var markerTitle = "<em>" + $markerInfo.club+"</em><br>";
            }
          }
        ...
        markerClosestClusterer.setIgnoreHidden(true);
        // CheckMarkerFilters(true);
        $('div.closetMarker').find('#loader').hide();
      });
    }
    ...
    function EraseClosestMarkers(){
      ...
      markerClosestClusterer = new MarkerClusterer(null);
      nearestMarkers = {};
    }
...
//*********************************************************************************************************************//
    ...
    function GetMarkerWeather($infoKey){
      $('#markerWeather').show();
      $('#loader').clone().appendTo('div#markerWeather').show();
      ...
      if($infoKey == "home"){
        ...
        // } 
        var curObjectCount = 0;
        for(coords in $markerLocation){
          if(curObjectCount == 0){
            $markerLocation = $location.LatLng[coords];
          } else if(curObjectCount == 1){
            $markerLocation = $markerLocation+','+ $location.LatLng[coords];
          }
          curObjectCount++;
        }
        curObjectCount = 0;
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          $('#weather').hide();
          $('#weather').show(500);
          $('div#markerWeather').find('#loader').hide();
        },
        error: function (e) {
...
//*********************************************************************************************************************//
      ...
      if((curRegion != $region && $.inArray($region, regionsClicked) ==-1) || $erased){
        for(markers in maps) { 
          ...
          if(markerRegion == $region){ 
            ...
            if(exists(maps[markers].title)){
              var markerTitle = "<h4><trong>"+maps[markers].title +" </strong></h4><em>"+maps[markers].club +"</em><br>";
            } else{
              var markerTitle = "<em>" + maps[markers].club+"</em><br>";
            }
        ...
        if(!foundMarkers) {
          ...
          for (var markers in markerArray) {
            if(!exists(markerArray[markers].markerElevation) || !markerArray[markers].markerElevation){
              getMarkersElevation(markerArray[markers].markerSlug,markerElevationArray);
              markerArray[markers].markerElevation = true;
            }
          }
    ...
    function createMarker($map, $options) {
      ...
      if(exists($markerInfo) && !$options.closest && !$options.closestContent){
        $mapContent = "";
        if(exists($markerInfo.mapInfo.adresse)){
          $mapContent = '<hr><em><strong>Adresse : </strong></em>' + $markerInfo.mapInfo.adresse;
        }
        $mapContent += "<br>(<em><strong>Coordonnées - </strong> Latitude : "+$markerInfo.mapInfo.latlng.lat.toFixed(2)+" , longitude : "+$markerInfo.mapInfo.latlng.lng.toFixed(2) + "</em>)";
        $options.content += $mapContent;
      }
        ...
        for (var i = 0; i < $functions.length; i++) {
          switch ($functions[i]) {
            ...
            case 'infowindow':
              ...
              if(!directionActiv && !closestActiv){
...
//*********************************************************************************************************************//
    ...
    $(".direction").click(function(){
      if(!directionActiv){
        ...
        $('.stopDirection').show();
        $('.stopDirection').text("Annuler");
        $('div#directionTuto div#directionClick').show();
      } else {
        ...
        $('.addDirection').hide();
        $('div#directionTuto div#directionClick').hide();
        $('div#directionTuto div#directionAdd').hide();
        $('div#directionTuto div#directionLaunch').hide();
      }
    });
...
<button style = "text-decoration : none; background : none; border : none;">
    <img style = "fill :red;" src="<?php echo Router::webroot("TravelModesWalking.svg"); ?>" alt="A sample SVG button."
...
<a href="http://tavmjong.free.fr/" target="_blank">
  ...
  <div class="col-md-5">
    ...
    <div id="directions-panel" style = "display : none;">
      <div id="directionTuto">
        <div id="directionClick" style = "display : none;">
          <p>
            Veuillez appuyer sur un "marker" pour le choisir comme point de départ/destination.
            Si vous voulez changer de point de départ/destination, appuyer sur un autre "marker" pour le choisir à la place du dernier.
            <br>
            Appuyer sur "Annuler" pour revenir au menu principal et annuler le service de direction.
          </p>
        </div>
        <div id="directionAdd" style = "display : none;">
          <p>
            Une fois vos points de départ/destination choisis, vous pouvez ajouter des points de passage (détours) par lesquels vous passerez en appuyant sur le bouton "Ajouter" puis sur les "marker" de votre choix. Il suffit d'appuyer sur le même "marker" pour annuler le choix.
          </p>
        </div>
        <div id="directionLaunch" style = "display : none;">
          <p>
            Pour finir, vous pouvez lancer le service de direction en appuyant sur le bouton "Lancer".
          </p>
        </div>
      </div>
      <div id="start"></div>
  ...
  <div class="col-md-7">
    ...
    <div id="localAdress" style = "display : none;">
        ...
        <p class="correctAddress"> Cette Adresse est elle correcte ?
          ...
          <br>
          Il n'est pas nécessaire que l'adresse trouvé soit correcte à 100%, il suffit que celle-ci soit proche de votre adresse réel pour que les services proposé par le site fonctionne. <br>
          Il est aussi possible que l'utilisation d'une extension pour bloquer les banniéres publicitaires (comme Adblock Plus) puissent interférer avec le service de géolocalisation, pour évitez cela veuillez désactiver le plugin ou l'autorisez pour le site de la FRMALS, celui-ci n'utilisant pas les publicités. <br>
          Dans le cas improbable où cela se produirait, n'hésitez à nous contactez pour que nous réglions que le probléme.
        </p>
    ...
    <div id="directionControles" style = "display : none;">
      <button class="stopDirection" style = "display : none;">Retour</button>
