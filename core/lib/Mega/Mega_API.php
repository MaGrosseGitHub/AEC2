<?php
    ...
    function api_req2($req, $params = array()) {
      global $seqno, $sid;
      ...
      $response = http_do_request($url, $payload);

      return json_decode($response);
    }

    function http_do_request($url, $payload) {
    ...
    function login($email, $password) {
      ...
      $res = api_req2(array($req))[0];
      // $res = api_req(array('a' => 'us', 'user' => $email, 'uh' => $uh));
      debug(array('a' => 'us', 'user' => $email, 'uh' => $uh), "request");
      debug($res);
      ...
      if (!empty($res->csid)) {
        ...
        return (array("sid" => $sid, "master_key" => $master_key, "rsa_priv_key" => $rsa_priv_key));
      }
    }
    ...
    function uploadfile($filename) {
      ...
      $size = strlen($data);
      debug($size);
      $ul_url = api_req2(array('a' => 'u', 's' => $size));
      debug($ul_url);
      $ul_url = $ul_url->p;
    ...
    class Mega_API {
        ...
        public function upload($email, $password, $filename){
